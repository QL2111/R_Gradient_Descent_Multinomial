...
data <- read.csv(file = "dataenedis.csv")
data
```
...
data$Etiquette_DPE <- as.factor(data$Etiquette_DPE)
...
prepared_data <- data_prep$prepare_data(data, "Etiquette_DPE", 0.7, stratify = TRUE)
...
model <- LogisticRegressionMultinomial$new(learning_rate = 0.1, num_iterations = 300,loss="logistique", optimizer="adam", use_early_stopping=TRUE)
