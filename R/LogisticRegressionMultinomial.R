...
LogisticRegressionMultinomial <- R6Class("LogisticRegressionMultinomial",
  public = list(
    ...
    predict = function(X) {
      X <- cbind(1, X)  # Add intercept term
      linear_model <- X %*% self$coefficients # logits
      probabilities <- self$softmax(linear_model)
    ...
    var_importance = function() {
      ...
      feature_names <- colnames(self$coefficients)[-1]  # Get the feature names
      
      # By class
      importance_scores <- rowMeans(coef_matrix)  # Average across classes
      importance_ranked <- sort(importance_scores, decreasing = TRUE) # Sort in descending order
      
      # Print the feature importance sorted
    ...
    #' @description Exports the trained model to a PMML (Predictive Model Markup Language) file. The PMML file contains the model's coefficients and metadata.
